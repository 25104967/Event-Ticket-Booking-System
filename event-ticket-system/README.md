...
- **Visual seat map builder** (`staff/venues.php`, `staff/venue-form.php`, `staff/venue-seatmap.php`):
  Admins and Organizers can now design a venue's actual seating layout instead of
  relying on a fixed rectangular grid. The builder lets you set a grid size, click
  cells to toggle them between a seat and an empty gap (for aisles), and "paint"
  a named section (e.g. "VIP", "Orchestra", "Balcony") onto groups of seats.
  Seats that already have a booking are locked (shown with 🔒) and can never be
  deleted, moved, or renumbered by a later edit — verified with an automated test
  that books a seat, edits the layout around it, and confirms it survives untouched.
- **Real visual seat map for customers** (`customer/booking.php`): the booking page
  now renders the actual layout designed by staff — true gaps where aisles were
  placed, and section labels grouping rows — instead of a plain uniform grid.
  Multi-seat selection (up to 6 per order) works the same as before.

- **Sales report & attendee list CSV exports** (`staff/export-sales.php`,
  `staff/export-attendees.php`): Admins and Organizers can download a CSV sales
  report (tickets sold and revenue per event, scoped the same way as the
  dashboard) and a full attendee list per event (name, email, tier, seat, QR
  code, check-in status, payment reference) — useful for box-office reporting
  or importing into a spreadsheet. While building this we also found and fixed
  a real revenue-calculation bug: multi-ticket orders share one Transaction
  record, so summing it once per booking row was silently multiplying revenue
  totals by the ticket count. Both the dashboard and these exports now compute
  revenue per distinct transaction, verified with an automated test that books
  a 3-ticket order and confirms the total is reported once, correctly.

- **Full venue CRUD** (`staff/venues.php`, `staff/venue-form.php`): venues can
  now be created, read, **updated** (edit name/address/capacity), and
  **deleted** — not just created. Deleting a venue is blocked with a clear
  message if any event still uses it (checked before touching the database,
  with a DB-level try/catch as a second line of defense); deleting an unused
  venue correctly cascades its seat map away with it. Verified with an
  automated test that edits a venue, confirms a venue with events can't be
  deleted, then deletes an empty one and confirms its seats are gone too.
...
- PDF exports (CSV is done); drag-free freeform seat positioning (the seat map
  builder is grid-based, not pixel-perfect freeform placement)
...
│   ├── api-add-venue.php          # AJAX endpoint: quick-add a venue (from event form)
│   ├── venues.php                 # venue list with seat-map status
│   ├── venue-form.php             # create a new venue
│   ├── venue-seatmap.php          # interactive visual seat map builder
│   ├── api-save-seatmap.php       # AJAX endpoint: save a venue's seat layout
│   ├── scan.php                   # QR check-in scanner (manual + camera)
│   ├── export-attendees.php       # CSV export: attendee list per event
│   ├── export-sales.php           # CSV export: sales report across events
