...
if ($tier_id) {
    ...
    if ($selected_tier) {
        ...
        if ($requires_seat) {
            ...
            $seats_stmt = $db->prepare('SELECT * FROM Seats WHERE Venue_ID = ? AND Is_Available = 1 ORDER BY Pos_Y, Pos_X');
            $seats_stmt->execute([$event['Venue_ID']]);
            $all_seats = $seats_stmt->fetchAll();

            $max_x = 0;
            foreach ($all_seats as $seat) {
                $section = $seat['Section_Label'] ?: 'General';
                $seats_by_row[$section][$seat['Pos_Y']]['label'] = $seat['Seat_Row'];
                $seats_by_row[$section][$seat['Pos_Y']]['seats'][$seat['Pos_X']] = $seat;
                $max_x = max($max_x, (int)$seat['Pos_X']);
            }
        }
    }
}
        ...
        <div class="panel">
          ...
          <?php if ($requires_seat): ?>
            ...
            <div class="seatmap-wrap">
              <?php foreach ($seats_by_row as $section => $rows): ksort($rows); ?>
                <?php if (count($seats_by_row) > 1): ?>
                  <div class="section-label"><?= e($section) ?></div>
                <?php endif; ?>
                <div class="seat-grid">
                  <?php foreach ($rows as $row): ?>
                    <div class="seat-row">
                      <span class="seat-row-label"><?= e($row['label']) ?></span>
                      <?php for ($x = 0; $x <= $max_x; $x++):
                          $seat = $row['seats'][$x] ?? null;
                          if (!$seat): ?>
                      ...
                      ?>
                        <button type="button" class="seat <?= $taken ? 'seat-taken' : '' ?>"
                                ...
                                title="<?= e($row['label'] . $seat['Seat_Number']) ?>">
